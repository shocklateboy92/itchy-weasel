...
, m_iStrengthValue(true)
, m_iThreatValue(true)
, m_iDemandResourceCounter(true)
, m_unitBeingBuiltForOperation()
, m_aiBaseYieldRateFromTerrain(true)
, m_abRevealed(true)
...
void CvCity::init(int iID, PlayerTypes eOwner, int iX, int iY, bool bBumpUnits)
{
	...
	setName(owningPlayer.getNewCityName());
	...
	owningPlayer.GetCityConnections()->Update();
...
void CvCity::reset(int iID, PlayerTypes eOwner, int iX, int iY, bool bConstructorCall)
{
	...
	m_iJONSCulturePerTurnFromTerrain = 0;
	...
	if (!bConstructorCall)
	{
		...
		m_paiNoResource.clear(); m_paiNoResource.resize(iNumResources);
		m_paiFreeResource.clear(); m_paiFreeResource.resize(iNumResources);
		m_paiNumResourcesLocal.clear(); m_paiNumResourcesLocal.resize(iNumResources);
		...
		m_paiProjectProduction.clear(); m_paiProjectProduction.resize(iNumProjectInfos);
		...
		m_paiSpecialistProduction.clear(); m_paiSpecialistProduction.resize(iNumSpecialistInfos);
		...
		m_paiUnitProduction.clear(); m_paiUnitProduction.resize(iNumUnitInfos);
		m_paiUnitProductionTime.clear(); m_paiUnitProductionTime.resize(iNumUnitInfos);
		m_paiGreatPeopleUnitRate.clear(); m_paiGreatPeopleUnitRate.resize(iNumUnitInfos);
		m_paiGreatPeopleUnitProgress.clear(); m_paiGreatPeopleUnitProgress.resize(iNumUnitInfos);
		...
		m_paiSpecialistCount.clear(); m_paiSpecialistCount.resize(iNumSpecialistInfos);
		m_paiMaxSpecialistCount.clear(); m_paiMaxSpecialistCount.resize(iNumSpecialistInfos);
		m_paiForceSpecialistCount.clear(); m_paiForceSpecialistCount.resize(iNumSpecialistInfos);
		m_paiFreeSpecialistCount.clear(); m_paiFreeSpecialistCount.resize(iNumSpecialistInfos);
		...
		m_paiImprovementFreeSpecialists.clear(); m_paiImprovementFreeSpecialists.resize(iNumImprovementInfos);
		...
		m_paiUnitCombatFreeExperience.clear(); m_paiUnitCombatFreeExperience.resize(iNumUnitCombatClassInfos);
		m_paiUnitCombatProductionModifier.clear(); m_paiUnitCombatProductionModifier.resize(iNumUnitCombatClassInfos);
		...
		m_paiFreePromotionCount.clear(); m_paiFreePromotionCount.resize(iNumPromotionInfos);
...
void CvCity::PreKill()
{
	...
	// Eject garrison out of this City.  If we don't it could try to grab the City pointer later and crash
	if (GetGarrisonedUnit() != NULL)
	{
		GetGarrisonedUnit()->UnGarrison();
	}
		...
		if (eResource != NO_RESOURCE)
		{
			...
			if (GET_PLAYER(getOwner()).getNumResourceTotal(eResource) > 0)
			{
				...
				if (pNotifications)
				{
					...
					pNotifications->Add(NOTIFICATION_DEMAND_RESOURCE, strText.toUTF8(), strSummary.toUTF8(), getX(), getY(), eResource);
				}
			}
		}
	}
...
void CvCity::processBuilding(BuildingTypes eBuilding, int iChange, bool bFirst, bool bObsolete, bool /*bApplyingAllCitiesBonus*/)
{
	...
	if (!(owningTeam.isObsoleteBuilding(eBuilding)) || bObsolete)
	{
		...
		int iBuildingCulture = pBuildingInfo->GetJONSCulture();
		...
		// Culture bonus for local resources
		int iCulture;
		ResourceTypes eResource;
		for (int iResourceLoop = 0; iResourceLoop < GC.getNumResourceInfos(); iResourceLoop++)
		{
...
{
	VALIDATE_OBJECT
	CvUnit* pReturnUnit = NULL;
	if (m_iGarrisonedUnit >= 0)
	{
		pReturnUnit = GET_PLAYER(m_eOwner).getUnit(m_iGarrisonedUnit);
	}
	return pReturnUnit;
}

//	--------------------------------------------------------------------------------
bool CvCity::SetGarrisonedUnit (CvUnit* pUnit)
{
	VALIDATE_OBJECT
	CvAssertMsg(m_iGarrisonedUnit < 0, "Trying to add a garrison unit when one is already set");
	if (m_iGarrisonedUnit < 0)
	{
		m_iGarrisonedUnit = pUnit->GetID();
		updateStrengthValue();
		// Garrisoned Units can affect Happiness, based on Policies
...
//	--------------------------------------------------------------------------------
void CvCity::EjectGarrisonedUnit ()
{
	VALIDATE_OBJECT
	if (m_iGarrisonedUnit >= 0)
	{
	...
	if (iOldPopulation != iNewValue)
	{
		...
		if (bReassignPop)
		{
			if (iPopChange < 0)
			{
				...
				{
					GetCityCitizens()->DoRemoveWorstCitizen();
				}
...
int CvCity::GetJONSCultureThreshold() const
{
	...
	// -50 = 50% cost
	int iModifier = GET_PLAYER(getOwner()).GetPlotCultureCostModifier() + m_iPlotCultureCostModifier;
	if (iModifier != 0)
	{
...
int CvCity::GetBaseJONSCulturePerTurn() const
{
	...
	iCulturePerTurn += GetJONSCulturePerTurnFromTerrain();
...
//	--------------------------------------------------------------------------------
int CvCity::GetJONSCulturePerTurnFromTerrain() const
{
	VALIDATE_OBJECT
	return m_iJONSCulturePerTurnFromTerrain;
}

//	--------------------------------------------------------------------------------
void CvCity::ChangeJONSCulturePerTurnFromTerrain(int iChange)
{
	VALIDATE_OBJECT
	if (iChange != 0)
	{
		m_iJONSCulturePerTurnFromTerrain = (m_iJONSCulturePerTurnFromTerrain + iChange);
	}
}
...
bool CvCity::DoRazingTurn()
{
	...
	if (IsRazing())
	{
		...
		// Counter is positive, reduce population by 1
		else
		{
...
{
	// If the city has less pop than Happiness, don't add it in!
	if (getPopulation() < GetBaseHappinessFromBuildings())
		return getPopulation() + GetUnmoddedHappinessFromBuildings();

	return GetBaseHappinessFromBuildings() + GetUnmoddedHappinessFromBuildings();
}
...
BuildingTypes CvCity::ChooseFreeCultureBuilding() const
{
	...
	for (int iI = 0; iI < iNumBuildingInfos; iI++)
	{
		...
		if(pkBuildingInfo)
		{
			...
			{
				int iCulture = pkBuildingInfo->GetJONSCulture();
...
void CvCity::updateStrengthValue()
{
	...
	if (plot()->isHills())
	{
		iStrengthMod += /*15*/ GC.getCITY_STRENGTH_HILL_MOD();
	}
...
void CvCity::setDamage(int iValue, bool noMessage)
{
	...
	if (iValue < 0)
		...
		if (iValue > GC.getMAX_CITY_HIT_POINTS())
			iValue = GC.getMAX_CITY_HIT_POINTS();
	...
	if (iValue != getDamage())
	{
		...
		if ( !noMessage && plot()->GetActiveFogOfWarMode() == FOGOFWARMODE_OFF )
		{
			char text[256];
			text[0] = NULL;
			int iNewValue = MIN(GC.getMAX_CITY_HIT_POINTS(),iValue);
		...
		for (iDY = -iMaxRange; iDY <= iMaxRange; iDY++)
		{
			...
			if (pLoopPlot != NULL)
			{
				...
				if (iInfluenceCost > 0)
				{
					...
					if (pLoopPlot->getResourceType(thisTeam) != NO_RESOURCE)
					{
					...
					{
						pAdjacentPlot = plotDirection(pLoopPlot->getX(), pLoopPlot->getY(), (DirectionTypes) iDirectionLoop);
...
int CvCity::GetBuyPlotCost(int iPlotX, int iPlotY) const
{
	...
	iDistance -= (GetCheapestPlotInfluence() - 1);	// Subtract one because we want 1 to be the lowest value possible

	int iInfluenceCost = iDistance * iPLOT_INFLUENCE_DISTANCE_MULTIPLIER;

	// Resource here?
	if (pPlot->getResourceType(getTeam()) != NO_RESOURCE)
	{
		int iPLOT_BUY_RESOURCE_COST =	/*-100*/ GC.getPLOT_BUY_RESOURCE_COST();
		iInfluenceCost += iPLOT_BUY_RESOURCE_COST;
	}

	if (iInfluenceCost > 100)
	{
		iCost *= iInfluenceCost;
		iCost /= 100;
	}
...
bool CvCity::CreateBuilding(BuildingTypes eBuildingType)
{
	...
	if (kPlayer.isBuildingClassMaxedOut(eBuildingClass, 1))
	{
...
bool CvCity::IsCanPurchase(bool bOnlyTestVisible, UnitTypes eUnitType, BuildingTypes eBuildingType, ProjectTypes eProjectType)
{
	...
	{
		if (!canTrain(eUnitType, false, bOnlyTestVisible))
			return false;
	...
	{
		if (!canConstruct(eBuildingType, false, bOnlyTestVisible))
		{
			bool bAlreadyUnderConstruction = canConstruct(eBuildingType, true, bOnlyTestVisible) && getFirstBuildingOrder(eBuildingType) != -1;
	...
	else if (eProjectType != NO_PROJECT)
	{
		...
		if (!canCreate(eProjectType, false, bOnlyTestVisible))
			return false;

		iGoldCost = GetPurchaseCost(eProjectType);
	}

	// Some items cannnot be purchased, e.g. Courthouse
	if (iGoldCost == -1)
		return false;
...
void CvCity::Purchase (UnitTypes eUnitType, BuildingTypes eBuildingType, ProjectTypes eProjectType)
{
	VALIDATE_OBJECT

	// Can we actually buy this thing?
	if (!IsCanPurchase(/*bOnlyTestVisible*/ false, eUnitType, eBuildingType, eProjectType))
		return;
...
//	--------------------------------------------------------------------------------
template<typename T> void FillOutArray(T& kArray, uint uiWantSize, int iFillValue)
{
	if (kArray.size() < uiWantSize)
	{
		uint uiOldSize = kArray.size();
		kArray.resize(uiWantSize);
		// Set any new DomainTypes to 0
		for (uint uiI = uiOldSize; uiI < uiWantSize; uiI++)
			kArray[uiI] = iFillValue;
...
void CvCity::read(FDataStream& kStream)
{
	...
	kStream >> m_iJONSCulturePerTurnFromTerrain;
	...
	else
	{
		...
		m_iResourceDemanded = CvInfosSerializationHelper::ConvertV0((ResourceTypes)(m_iResourceDemanded), CVINFO_V0_TAGS(CvInfosSerializationHelper::ms_V0ResourceTags));
	}
	...
	FillOutArray(m_aiDomainFreeExperience, NUM_DOMAIN_TYPES, 0);
	FillOutArray(m_aiDomainProductionModifier, NUM_DOMAIN_TYPES, 0);
	...
	if (uiVersion >= 14)
	{
		CvInfosSerializationHelper::ReadHashedDataArray(kStream, m_paiNoResource);
		CvInfosSerializationHelper::ReadHashedDataArray(kStream, m_paiFreeResource);
		CvInfosSerializationHelper::ReadHashedDataArray(kStream, m_paiNumResourcesLocal);
	}
	else
	{
		std::vector<int> aTemp;
		CvInfosSerializationHelper::ReadV0DataArray<int>(kStream, aTemp, GC.getNumResourceInfos(), CVINFO_V0_TAGS(CvInfosSerializationHelper::ms_V0ResourceTags) ); m_paiNoResource = aTemp;
		CvInfosSerializationHelper::ReadV0DataArray<int>(kStream, aTemp, GC.getNumResourceInfos(), CVINFO_V0_TAGS(CvInfosSerializationHelper::ms_V0ResourceTags)); m_paiFreeResource = aTemp;
		CvInfosSerializationHelper::ReadV0DataArray<int>(kStream, aTemp, GC.getNumResourceInfos(), CVINFO_V0_TAGS(CvInfosSerializationHelper::ms_V0ResourceTags)); m_paiNumResourcesLocal = aTemp;
	}
		...
		for (int iI = 0; iI < iNumEntries; iI++)
		{
			...
			if (sTemp != "NO_UNIT")
			{
				...
				if (iType != -1)
				{
					int iTempValue;
					kStream >> iTempValue;
					m_paiUnitProduction[iType] = iTempValue;
					kStream >> iTempValue;
					m_paiUnitProductionTime[iType] = iTempValue;
					kStream >> iTempValue;
					m_paiGreatPeopleUnitRate[iType] = iTempValue;
					kStream >> iTempValue;
					m_paiGreatPeopleUnitProgress[iType] = iTempValue;
				}
				else
				{
					CvString szError;
	...
	else
	{
		...
		{
			kStream >> iTempValue;
			m_paiUnitProduction[iI] = iTempValue;
		}
		kStream >> iNumEntries;
		for (int iI = 0; iI < iNumEntries; iI++)
		{
			kStream >> iTempValue;
			m_paiUnitProductionTime[iI] = iTempValue;
		}
		kStream >> iNumEntries;
		for (int iI = 0; iI < iNumEntries; iI++)
		{
			kStream >> iTempValue;
			m_paiGreatPeopleUnitRate[iI] = iTempValue;
		}
		kStream >> iNumEntries;
		for (int iI = 0; iI < iNumEntries; iI++)
		{
			kStream >> iTempValue;
			m_paiGreatPeopleUnitProgress[iI] = iTempValue;
		}
	}
	...
	if (uiVersion <= 12)
		CvInfosSerializationHelper::ReadV0DataArray(kStream, m_ppaiResourceYieldChange, NUM_YIELD_TYPES, GC.getNumResourceInfos(), CVINFO_V0_TAGS(CvInfosSerializationHelper::ms_V0ResourceTags));
	else
...
void CvCity::write(FDataStream& kStream) const
{
	...
	kStream << m_iThreatValue;
	kStream << m_iGarrisonedUnit;
	CvInfosSerializationHelper::WriteHashed(kStream, (ResourceTypes)(m_iResourceDemanded));
	kStream << m_iWeLoveTheKingDayCounter;
	kStream << m_iLastTurnGarrisonAssigned;
	kStream << m_iThingsProduced;
	kStream << m_iDemandResourceCounter;
...
bool CvCity::canRangeStrike() const
{
	...
	if (getDamage() == GC.getMAX_CITY_HIT_POINTS())
		return false;
...
//	--------------------------------------------------------------------------------
/// Does this City have a Garrisoned Unit that allows it to Range Strike?
bool CvCity::IsHasGarrisonThatAllowsRangeStrike() const
{
	VALIDATE_OBJECT
	CvUnit* pUnit = GetGarrisonedUnit();
	if (NULL != pUnit && pUnit->isRanged())
	{
		return true;
	}
...
int CvCity::rangeCombatUnitDefense(const CvUnit* pDefender) const
{
	...
	if (pDefender->GetMaxRangedCombatStrength(NULL, /*pCity*/ NULL, false, false) > 0 && !pDefender->getDomainType() == DOMAIN_SEA)
	{
...
int CvCity::GetAirStrikeDefenseDamage(const CvUnit* pAttacker, bool bIncludeRand) const
{
	...
	int iDefenderDamageRatio = GC.getMAX_CITY_HIT_POINTS() - getDamage();
	int iDefenderDamage = /*200*/ GC.getAIR_STRIKE_SAME_STRENGTH_MIN_DEFENSE_DAMAGE() * iDefenderDamageRatio / GC.getMAX_CITY_HIT_POINTS();
	...
	if (bIncludeRand)
	{
		...
		iDefenderRoll /= GC.getMAX_CITY_HIT_POINTS();
	}
	else
	{
