...
public:
	...
	int ComputeCultureFromImprovement(CvImprovementEntry &kImprovement, ImprovementTypes eValue);
	...
	PlotTypes getPlotType() const { return (PlotTypes)m_ePlotType; }
	bool isWater()          const { return (PlotTypes)m_ePlotType == PLOT_OCEAN; };
	bool isHills()          const { return (PlotTypes)m_ePlotType == PLOT_HILLS; };
	bool isOpenGround()     const { if((PlotTypes)m_ePlotType == PLOT_HILLS || (PlotTypes)m_ePlotType == PLOT_MOUNTAIN || m_bRoughFeature) return false; return true; }
	bool isMountain()       const { return (PlotTypes)m_ePlotType == PLOT_MOUNTAIN; };

	TerrainTypes getTerrainType() const { return (TerrainTypes)m_eTerrainType; }
	...
	void setResourceType(ResourceTypes eNewValue, int iResourceNum);
