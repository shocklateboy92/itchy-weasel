...
HandicapTypes lastHumanHandicap(PlayerTypes p)
{
	...
	void readArchive(FDataStream& loadFrom, bool bReadVersion)
	{
		...
		if (uiVersion <= 1)						// Read the civilization indices.  Newer versions have the keys in the header.
		...
		else
		{
			...
			if ( (eGameType == GAME_HOTSEAT_NEW) || (eGameType == GAME_HOTSEAT_SCENARIO) || (eGameType == GAME_HOTSEAT_LOAD) )
				...
				if ( (eGameType == GAME_MP_NEW) || (eGameType == GAME_MP_SCENARIO) || (eGameType == GAME_MP_LOAD) )
					s_gameType = GAME_NETWORK_MULTIPLAYER;
				else
					if ( (eGameType == GAME_SP_NEW) || (eGameType == GAME_SP_SCENARIO) || (eGameType == GAME_SP_LOAD) )
		...
		if (uiVersion <= 1)						// Read the leader indices.  Newer versions have the keys in the header.
