...
public:
	...
	bool canBuild(const CvPlot* pPlot, BuildTypes eBuild, bool bTestEra = false, bool bTestVisible = false, bool bTestGold = true) const;
	...
	void ReportCultureFromKills(int iX, int iY, int iCulture, bool bWasBarbarian);
	void ReportGoldFromKills(int iX, int iY, int iGold);
	...
	void changeHappinessFromBuildings(int change);
	...
	void changeGetMinorFriendshipMinimum(int iChange);
	...
	void SetEmbarkedGraphicOverride(CvString szGraphicName) {m_strEmbarkedGraphicOverride = szGraphicName;};
	const CvString& GetEmbarkedGraphicOverride() const {return m_strEmbarkedGraphicOverride;};
...
protected:
	...
	int m_iJONSCulturePerTurnFromMinorCivs;
	...
	int m_iDomesticGreatGeneralRateModFromBldgs;
	...
	std::vector<int> m_paiResourceExport;
	std::vector<int> m_paiResourceImport;
	...
	std::vector< Firaxis::Array< int, NUM_YIELD_TYPES > > m_ppaaiBuildingClassYieldMod;
