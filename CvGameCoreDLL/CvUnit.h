...
struct CvUnitCaptureDefinition
{
	...
	CvUnitCaptureDefinition() : eOriginalOwner(NO_PLAYER), eOldPlayer(NO_PLAYER), eOldType(NO_UNIT), eCapturingPlayer(NO_PLAYER), eCaptureUnitType(NO_UNIT), bEmbarked(false) { }
	inline bool IsValid() const { return eCapturingPlayer != NO_PLAYER && eCaptureUnitType != NO_UNIT; }
};
...
public:
	...
	enum {
		MOVEFLAG_ATTACK						  = 0x01,
		MOVEFLAG_DECLARE_WAR				  = 0x02,
		MOVEFLAG_DESTINATION				  = 0x04,
		MOVEFLAG_NOT_ATTACKING_THIS_TURN	  = 0x08,
		MOVEFLAG_IGNORE_STACKING			  = 0x10,
		MOVEFLAG_PRETEND_EMBARKED			  = 0x20, // so we can check movement as if the unit was embarked
		MOVEFLAG_PRETEND_UNEMBARKED			  = 0x40, // to check movement as if the unit was unembarked
		MOVEFLAG_PRETEND_CORRECT_EMBARK_STATE = 0x80, // check to see if the unit can move into the tile in an embarked or unembarked state
	};
	...
	bool CanGarrison(const CvPlot* pPlot, bool bTestVisible = false) const;
	...
	int GetDeployFromOperationTurn() const {return m_iDeployFromOperationTurn;};
	void SetDeployFromOperationTurn(int iTurn) {m_iDeployFromOperationTurn = iTurn;};
	...
	/// Does the unit have a combat strength?
...
protected:
	...
	int m_iEmbarkedNotCivilianCount;
