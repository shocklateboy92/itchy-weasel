	...
	for( int iPolicy = 0; iPolicy < iPolicyCount; ++iPolicy )
	{
		...
		if( kSavedPlayer.m_kPolicies[iPolicy] )
		{
			...
			if( pkPolicy != NULL && pkPlayerPolicies != NULL )
			{
				...
				{
					pkPlayerPolicies->SetPolicyBranchUnlocked(ePolicyBranch, true);
...
void SetupCity(const CvWorldBuilderMap::City &kSavedCity, int iPlotX, int iPlotY)
{
	...
	if( ePlayer != NO_PLAYER )
	{
		...
		{
			Localization::String strCityName = Localization::Lookup(kSavedCity.m_szName);
			pkGameplayCity->setName(strCityName.toUTF8());
...
void SetupUnit(const CvWorldBuilderMap::Unit &kSavedUnit, int iPlotX, int iPlotY)
{
	...
	if( ePlayer != NO_PLAYER )
	{
		...
		if( pkGameplayUnit != NULL )
		{
			...
			if( kSavedUnit.GetFlag(CvWorldBuilderMap::Unit::UNIT_GARRISONED) )
			{
				CvPlot *pkPlot = pkGameplayUnit->plot();
				if( pkPlot != NULL && pkGameplayUnit->CanGarrison(pkPlot) )
					pkGameplayUnit->Garrison(pkPlot);
			}
			...
			if( kSavedUnit.m_hCustomName.Valid() )
			{
				...
				if( pkUnitName != NULL )
				{
					Localization::String strName = Localization::Lookup(pkUnitName->m_szText);
					pkGameplayUnit->setName(strName.toUTF8());
				}
			}
		}
	}
	...
	for( int iDist = 1; iDist < iMapHeight || iDist < iMapWidth; ++iDist )
	{
		...
		for( int iRow = 1; iRow < iDist; ++iRow )
		{
			...
			{	// Right-most plot in the current row
				...
				{
					if( bWorldWrap )
						do { iX -= iMapWidth; } while( iX >= iMapWidth );
				}
