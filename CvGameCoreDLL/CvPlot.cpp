...
bool CvPlot::canBuild(BuildTypes eBuild, PlayerTypes ePlayer, bool bTestVisible) const
{
	...
	// Can't clear features in the lands of players that aren't on our team or that we're at war with
	if (getFeatureType() != NO_FEATURE)
	{
...
//	--------------------------------------------------------------------------------
int CvPlot::GetCulture() const
{
	return m_iCulture;
}

//	--------------------------------------------------------------------------------
void CvPlot::ChangeCulture(int iChange)
{
	if (iChange != 0)
	{
		m_iCulture += iChange;

		// If a city is working this plot, update their culture
		CvCity* pWorkingCity = getWorkingCity();
		if (pWorkingCity != NULL)
		{
			if (pWorkingCity->GetCityCitizens()->IsWorkingPlot(this))
			{
				pWorkingCity->ChangeJONSCulturePerTurnFromTerrain(iChange);
			}
		}
	}
}
...
//	--------------------------------------------------------------------------------
int CvPlot::ComputeCultureFromImprovement(CvImprovementEntry &kImprovement, ImprovementTypes eValue)
{
	CvPlot *pAdjacentPlot;
	int iRtnValue = 0;

	iRtnValue += kImprovement.GetCulture();

	if (iRtnValue > 0 && getOwner() != NO_PLAYER)
	{
		iRtnValue += GET_PLAYER(getOwner()).GetPlayerPolicies()->GetNumericModifier (POLICYMOD_EXTRA_CULTURE_FROM_IMPROVEMENTS);
		iRtnValue += GET_PLAYER(getOwner()).GetPlayerPolicies()->GetImprovementCultureChange((ImprovementTypes)kImprovement.GetID());
	}
...
CvPlot* CvPlot::getInlandCorner() const
{
	...
	for (int iI = 0; iI < 4; ++iI)
	{
		...
		{
		case 0:
			pRiverPlot = GC.getMap().plotCheckInvalid(getX(), getY()); break;
		case 1:
...

		{
			...
			if (isOwned())
			{
				...
				if (eFeature != NO_FEATURE)
				{
					...
					{
						int iCulture = GC.getFeatureInfo(eFeature)->getCulture();
						if (GET_PLAYER(getOwner()).GetPlayerTraits()->GetNaturalWonderYieldModifier() > 0)
						{
							iCulture *= (100 + GET_PLAYER(getOwner()).GetPlayerTraits()->GetNaturalWonderYieldModifier());
							iCulture /= 100;
						}
			...
			if (isOwned())
			{
				...
				if (eFeature != NO_FEATURE)
				{
					...
					{
						int iCulture = GC.getFeatureInfo(eFeature)->getCulture();
						if (GET_PLAYER(eNewValue).GetPlayerTraits()->GetNaturalWonderYieldModifier() > 0)
						{
							iCulture *= (100 + GET_PLAYER(getOwner()).GetPlayerTraits()->GetNaturalWonderYieldModifier());
							iCulture /= 100;
						}
...
//	--------------------------------------------------------------------------------
void CvPlot::setResourceType(ResourceTypes eNewValue, int iResourceNum)
{
...
void CvPlot::setImprovementType(ImprovementTypes eNewValue)
{
	...
	if (eOldImprovement != eNewValue)
	{
		...
		if (m_eImprovementType != NO_IMPROVEMENT)
		{
			...
			// Culture from Improvement
			int iCulture = ComputeCultureFromImprovement(newImprovementEntry, eNewValue);
			if (iCulture != 0)
			{
				ChangeCulture(iCulture);
			}
...
{
	// Remove current culture
	int iOldCulture = GetCulture();
	ChangeCulture(-iOldCulture);

	// Readd with new value
	ImprovementTypes eImprovement = getImprovementType();
	if (eImprovement != NO_IMPROVEMENT)
	{
		CvImprovementEntry& kImprovementEntry = *GC.getImprovementInfo(eImprovement);

		// Culture from Improvement
		int iCulture = ComputeCultureFromImprovement(kImprovementEntry, eImprovement);
		if (iCulture != 0)
		{
			ChangeCulture(iCulture);
		}
		...
		if (bOldVisible != isVisible(eTeam))
		{
			...
			if (isVisible(eTeam))
			{
				...
				for (int iUnitLoop = 0; iUnitLoop < getNumUnits(); iUnitLoop++)
				{
					...
					GET_TEAM(eTeam).meet(loopUnit->getTeam());
				}
				...
				if (isCity())
				{
					...
					{
						GET_TEAM(eTeam).meet(getTeam());	// If there's a City here, we can assume its owner is the same as the plot owner
					}
				}
			}
			else
	...
	if (pCity != NULL)
	{
		...
		for (iI = 0; iI < GC.getNumBuildingInfos(); ++iI)
		{
			...
			if(pkBuildingInfo)
			{
				...
				{
					//				pArea->changeBorderObstacleCount(pCity->getTeam(), ((GC.getBuildingInfo((BuildingTypes)iI)->IsAreaBorderObstacle()) ? iChange * pCity->GetCityBuildings()->GetNumActiveBuilding((BuildingTypes)iI) : 0));
...
int CvPlot::calculateMaxYield(YieldTypes eYield) const
{
	...
	{
		const ImprovementTypes eImprovement = static_cast<ImprovementTypes>(iImprovement);
		CvImprovementEntry* pkImprovementInfo = GC.getImprovementInfo(eImprovement);
		if(pkImprovementInfo)
		{
			iImprovementYield = std::max(calculateImprovementYieldChange(eImprovement, eYield, NO_PLAYER, true), iImprovementYield);
		}
	}
	...
	{
		const RouteTypes eRoute = static_cast<RouteTypes>(iRoute);
		CvRouteInfo* pkRouteInfo = GC.getRouteInfo(eRoute);
		if(pkRouteInfo)
		{
			iRouteYield = std::max(pkRouteInfo->getYieldChange(eYield), iRouteYield);
		}
	}
	...
	if (isWater() && !isImpassable())
	{
		...
		if (getResourceType() != NO_RESOURCE)
		{
			...
			for (int iBuilding = 0; iBuilding < GC.getNumBuildingInfos(); iBuilding++)
			{
				...
				if(pkBuildingInfo)
				{
					iBuildingYield = std::max(pkBuildingInfo->GetSeaResourceYieldChange(eYield), iBuildingYield);
				}
			}
	...
	if (isRiver())
	{
		...
		for (int iBuilding = 0; iBuilding < GC.getNumBuildingInfos(); iBuilding++)
		{
			...
			if(pkBuildingInfo)
			{
				iBuildingYield = std::max(pkBuildingInfo->GetRiverPlotYieldChange(eYield), iBuildingYield);
			}
		}
		iMaxYield += iBuildingYield;
	}

	// Extra yield from Features
	FeatureTypes eFeature = getFeatureType();
	if (eFeature != NO_FEATURE)
	{
		int iBuildingYield = 0;
		for (int iBuilding = 0; iBuilding < GC.getNumBuildingInfos(); iBuilding++)
		{
			...
			if(pkBuildingInfo)
			{
				iBuildingYield += std::max(iBuildingYield, pkBuildingInfo->GetFeatureYieldChange(eFeature, eYield));
	...
	// Extra yield from resources
	const ResourceTypes eTeamResource = getResourceType(getTeam());
	CvResourceInfo* pkTeamResourceInfo = NULL;
	if(eTeamResource != NO_RESOURCE)
		pkTeamResourceInfo = GC.getResourceInfo(eTeamResource);

	if (pkTeamResourceInfo)
	{
		int iBuildingYield = 0;
		for (int iBuilding = 0; iBuilding < GC.getNumBuildingInfos(); iBuilding++)
		{
			...
			if(pkBuildingInfo)
			{
				iBuildingYield += std::max(iBuildingYield, pkBuildingInfo->GetResourceYieldChange(eTeamResource, eYield));
			}
		}
		iMaxYield += iBuildingYield;
	}

	// Extra yield from traits
	int iExtraYieldThreshold = 0;
	for (int iTrait = 0; iTrait < GC.getNumTraitInfos(); iTrait++)
	{
		const TraitTypes eTrait = static_cast<TraitTypes>(iTrait);
		CvTraitEntry* pkTraitInfo = GC.getTraitInfo(eTrait);
		if(pkTraitInfo)
		{
			iExtraYieldThreshold  = std::max(pkTraitInfo->GetExtraYieldThreshold(eYield), iExtraYieldThreshold);

			if (pkTeamResourceInfo)
			{
				if (pkTeamResourceInfo->getResourceUsage() == RESOURCEUSAGE_STRATEGIC)
				{
					iMaxYield += pkTraitInfo->GetYieldChangeStrategicResources(eYield);
				}
			}
		}
	}
